...
def find_matching_milestone(
    ...
    Returns:
        Milestone ID if found, otherwise None
    """
    ...
    while has_more and page <= 10:  # GitHub returns max 1000 items
        ...
        if labels is not None:
            if isinstance(labels, str):
                filter_labels = [labels.lower()]
            else:
                filter_labels = [lb.lower() for lb in labels]

            filtered_items = []
            for item in items_page:
                item_labels = [label["name"].lower() for label in item["labels"]]

                match_all = True
                for required in filter_labels:
                    found = any(required in lb for lb in item_labels)
                    if not found:
                        match_all = False
                        break
...
github_tools = oxy.FunctionHub(name="github_apis", timeout=900)
...
def get_github_issues_api(
        ...
        version_str: Optional[str] = Field(None,
                                           description="Version/milestone string to auto-find matching milestone e.g. v1.2.1"),
        state: str = Field("closed", description="State of items",
                           enum=["open", "closed"]),
    ...
    Note: The title_contains filter performs local filtering after
    fetching results from GitHub.
    """
    # Auto-find milestone ID from version string if needed
    resolved_milestone = None
    version_str = get_default_value(version_str)
    if version_str:
        resolved_milestone = find_matching_milestone(
            repo=repo,
            version_str=version_str,
            exact_match=False
        )
        if resolved_milestone is not None:
            print(f"Automatically resolved version '{version_str}' to milestone ID {resolved_milestone}")
        else:
            print(f"Warning: Could not find milestone for version '{version_str}', proceeding without milestone filter")

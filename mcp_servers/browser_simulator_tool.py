...
16. `rotate_3d_model()`: Scroll left the page(or 3D model). rotate 3d model
"""
...
class BaseBrowser:
    def __init__(self, headless=True, cache_dir: Optional[str] = None):
        ...
        self.cache_dir = os.getenv('LOCAL_CACHE_DIR') if cache_dir is None else cache_dir
    ...
    def rotate_3d_model(self,
        ...
        用途：适用于任何需要持续方向键操作的3D模型查看器或CAD界面

        参数:
        direction: 旋转方向 ("left", "right", "up", "down")，默认为"left"
        duration: 按住键的持续时间（秒），默认为2秒
        """
        # 方向键映射
        key_map = {
            "left": "ArrowLeft",
        ...
        if direction not in key_map:
            raise ValueError(f"无效方向: {direction}. 请使用 'left', 'right', 'up' 或 'down'")
...
class AsyncBaseBrowser:
    ...
    async def async_visit_page(self, url: str, timeout: int = 300000, max_retries: int = 2) -> None:
    ...
    def visit_page(self, url: str, timeout: int = 300000, max_retries: int = 2) -> None:
    ...
    async def async_rotate_3d_model(
            self,
            direction: str = "left",
            duration: float = 2.0
    ) -> None:
        """
        持续点击屏幕中心并按住方向键旋转3D模型

        参数:
        direction: 旋转方向 ("left", "right", "up", "down")，默认为"left"
        duration: 操作持续时间（秒），默认为2秒
        """
        # 方向键映射
        key_map = {
            "left": "ArrowLeft",
        ...
        try:
            ...
            # 创建并维持操作状态
            await self._maintain_actions_state(duration, key)

        except Exception as e:
            logger.error(f"3D模型旋转出错: {e}")
            raise
    ...
    async def _maintain_actions_state(self, duration: float, key) -> None:
        """
        维持操作状态，确保动作持续执行

        使用独立协程持续发送动作信号，防止操作中断
        """
        start_time = asyncio.get_event_loop().time()

        # 持续发送动作直到达到指定持续时间
        while (asyncio.get_event_loop().time() - start_time) < duration:
            # 模拟持续动作（非必需的，但可以帮助某些系统维持状态）
            await self.page.mouse.down(button="left")
            await self.page.keyboard.down(key)
    ...
    def rotate_3d_model(self) -> None:
        r"""Scroll up the page."""
        return self.async_rotate_3d_model()
...
class AsyncBrowserToolkit(BaseToolkit):
    ...
    def _initialize_agent(self) -> Tuple["ChatAgent", "ChatAgent"]:
        ...
        if self.planning_agent_model is None:
            planning_model = ModelFactory.create(
                ...
                model_type=os.getenv('DEEPSEEK_V3'),
    ...
    async def async_observe(
        ...
        - Download file priority: csv > pdf
    ...
    async def async_act(self, action_code: str) -> Tuple[bool, str]:
        ...
        print(code, action_code, async_flag, feedback_flag)
        try:
...
_ASYNC_BROWSER_DESCRIPTION = """Performs comprehensive research on a topic through multi-level web searches and content analysis. 
Returns a structured summary of findings with source attribution and relevance ratings."""

mcp = FastMCP('a1', port=9010)
from oxygent import oxy
abs = oxy.FunctionHub(name="async_advanced_browser_tool", timeout=900)


@abs.tool(description=_ASYNC_BROWSER_DESCRIPTION)
async def async_advanced_browser_api(
    task_prompt: str = Field(description="The research query to explore."),
    start_url: str = Field(description="The url."),
    ) -> str:
    try:
        abt = AsyncBrowserToolkit()
        result = await asyncio.wait_for(abt.browse_url(task_prompt, start_url), timeout=1800)
        return result
    except asyncio.TimeoutError:
        # 超时处理逻辑
        return "Research operation timed out after 15 minutes"
...
if __name__ == "__main__":
    ...
    print(asyncio.run(async_advanced_browser_api(
        task_prompt='What was the day in 2024 when the official CNS twitter account posted a photo with four ELON MUSK? .',
        start_url="https://x.com/cns_update"
    )))

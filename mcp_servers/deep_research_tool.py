...
from mcp_servers.web_searcher_tool import WebSearcherTool
...
def convert_tool_to_api_format(tool_class) -> dict:
    return {
        "type": "function",
        "function": {
...
class OptimizedQueryTool:
    ...
    def forward(self, query: str, filter_year: Optional[int] = None):
...
class GenerateFollowUpsTool:
    ...
    def forward(self, follow_up_queries: List[str]) -> List[str]:
        """Generate follow-up queries based on insights."""
        # Placeholder for actual generation logic
        # In a real implementation, this would involve LLM interactions
        return follow_up_queries

class ExtractInsightsTool:
    ...
    output_type = "any"
    def forward(self, insights: any) -> any:
...
class DeepResearcherTool:
    ...
    async def forward(self, query: str,):
        ...
        try:
            optimized_query, filter_year = self._generate_optimized_query(query)
        ...
        output = self._summary(query, reference.output)

        return output

    def _generate_optimized_query(self, query: str) -> Tuple[str, Optional[int]]:
        ...
        try:
            ...
            tools = [
                convert_tool_to_api_format(OptimizedQueryTool())
            ]
    ...
    async def _research_graph(
        ...
        # 2. Extract insights
        new_insights = self._extract_insights(
            context,
            search_results,
            context.query,
            deadline
    ...
    def _extract_insights(
        ...
        for rst in results:
            ...
            insights = self._analyze_content(
    ...
    def _generate_follow_ups(
        ...
        tools = [
            convert_tool_to_api_format(GenerateFollowUpsTool())
        ]
    ...
    def _analyze_content(
        ...
        tools = [
            convert_tool_to_api_format(ExtractInsightsTool())
        ]
    ...
    def _summary(self, query: str, reference_materials: str) -> str:
...
dr = oxy.FunctionHub(name="deep_researcher_tool", timeout=900)


@dr.tool(description=_DEEP_RESEARCHER_DESCRIPTION)
# @mcp.tool(description=_DEEP_RESEARCHER_DESCRIPTION)
async def deep_researcher_api(
    query: str = Field(description="The research query to explore."),
    ) -> str:
    dr_tool = DeepResearcherTool()
    try:
        return await asyncio.wait_for(dr_tool.forward(query), timeout=1800)
...
if __name__ == "__main__":
    ...
    print(asyncio.run(deep_researcher_api('AI regulation paper submitted to arXiv.org in June 2022')))
    print("running")

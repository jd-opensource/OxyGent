...
from examples.gaia.dataset.huggingface import GAIADataset
from examples.gaia.dataset.load_and_save import init_files, load_processed, save_result
import pandas as pd
...
oxy_workspace = [
    ...
    oxy.ReActAgent(
        ...
        desc='WIKI HISTORIAN: Provides Wikipedia content/revisions at specific dates. FEATURES: Edit counts, infobox '
             'values, revision comparisons. REQUIRES: Title that complies with Wikipedia format standards',
    ...
    oxy.ReActAgent(
        name='result_agent',
        desc='FINISHER: Applies unit conversion(kg→tons), scaling(hundred→thousand) and formatting. INPUT: Must contain '
             'initial answer.',
        llm_model=os.getenv('DEEPSEEK_V3'),
...
async def run_main():
    ...
    result_dir = Path(os.getenv('LOCAL_RESULT_DIR'))
    ...
    datasets = GAIADataset(
        path="Your GAIA Dataset",
        name="2023_all",
        split="validation"
    )
    logger.info(f"Loaded dataset: {len(datasets)} examples.")
    datasets = [
        dataset for dataset in datasets if not (dataset['task_id'] in processed and
                                           processed[dataset['task_id']] == 'success')]
    if len(datasets) != 0:
        datasets = pd.concat(datasets, axis=1).T
    logger.info(f"To doing: {len(datasets)} tasks.")
    cnt = 0
    failed_tasks = []
    for idx, dataset in datasets.iterrows():
        cnt += 1
        logger.info(f"Remaining: {len(datasets) - cnt} tasks.")
        response = ''
        try:
            if dataset['task_id'] in failed_tasks:
                continue
            if dataset['task_id'] in processed and processed[dataset['task_id']] == 'success':
                continue
            logger.info(f"+++++++++++++Doing task: {dataset['question']} .++++++++++++++")
            if dataset['file_name']:
                query = [
                    {"type": "file_name",
                     "file_name": {"file": dataset['file_name']}},
                ]
            else:
                query = []
            query += [
            ...
            response = oxy_response.output
            logger.info(f"LLM: {oxy_response.output}")
            response = parse_result(response.split("```json")[1] if "```json" in response else response)
            if type(response) == str:
                response = eval(response)
            is_error = False
            save_result(dataset, response['arguments']['answer'], result_dir, checkpoint_file, failed_checkpoint_file,
                        is_error=is_error)
        except Exception as e:
            save_result(dataset, response, result_dir, checkpoint_file, failed_checkpoint_file, is_error=True)
            logger.info(e)
